...
BorderColor.prototype = {
   ...
   setHue: function(h) {
      ...
      // convert back to RGB...
      this.rgb = BorderColor.HSBtoRGB(hsb.h, hsb.s, hsb.b);
   },
   ...
   darken: function(percent) {
      var hsb  = this.asHSB();
      this.rgb = BorderColor.HSBtoRGB(hsb.h, hsb.s, Math.max(hsb.b - percent,0));
   },

   brighten: function(percent) {
      var hsb  = this.asHSB();
      this.rgb = BorderColor.HSBtoRGB(hsb.h, hsb.s, Math.min(hsb.b + percent,1));
   },
   ...
   asHSB: function() {
      return BorderColor.RGBtoHSB(this.rgb.r, this.rgb.g, this.rgb.b);
   },
...
BorderColor.createFromHex = function(hexCode) {
  if(hexCode.length==4) {
  ...
  return new BorderColor( parseInt(red,16), parseInt(green,16), parseInt(blue,16) );
}
 ...
 * an HTML element.
 */
BorderColor.createColorFromBackground = function(elem) {

   if (!elem.style) return new BorderColor(255,255,255);
   ...
   if ( actualColor.match(/^(transparent|rgba\(0,\s*0,\s*0,\s*0\))$/i) && elem.parentNode )
      return BorderColor.createColorFromBackground(elem.parentNode);

   if (actualColor == null) return new BorderColor(255,255,255);

   if ( actualColor.indexOf("rgb(") == 0 ) {
      var colors = actualColor.substring(4, actualColor.length - 1 );
      var colorArray = colors.split(",");
      return new BorderColor( parseInt( colorArray[0] ),
                            parseInt( colorArray[1] ),
                            parseInt( colorArray[2] )  );

   }
   else if ( actualColor.indexOf("#") == 0 ) {
      return BorderColor.createFromHex(actualColor);
   }
   else
      return new BorderColor(255,255,255);
}

BorderColor.HSBtoRGB = function(hue, saturation, brightness) {
 ...
 */
BorderColor.RGBtoHSB = function(r, g, b) {
...
BorderColor.createGradientV = function(e,startColor,endColor) {
  var c1=typeof(startColor)=='string' ? BorderColor.createFromHex(startColor) : startColor;
  var c2=typeof(endColor)=='string' ? BorderColor.createFromHex(endColor) : endColor;
  if (Prototype.Browser.IE) {
    e.style.filter = "progid:DXImageTransform.Microsoft.Gradient(GradientType=0,StartColorStr=\"" + c1.asHex() + "\",EndColorStr=\"" + c2.asHex() + "\")";
  } else {
    colorArray = BorderColor.createColorPath(c1,c2,Math.min(e.offsetHeight,50));
    var remh=e.offsetHeight,l=colorArray.length;
    var div=BorderColor.createGradientContainer();
...
BorderColor.createGradientH = function(e,startColor,endColor) {
  var c1=typeof(startColor)=='string' ? BorderColor.createFromHex(startColor) : startColor;
  var c2=typeof(endColor)=='string' ? BorderColor.createFromHex(endColor) : endColor;
  if (Prototype.Browser.IE) {
    e.style.filter = "progid:DXImageTransform.Microsoft.Gradient(GradientType=1,StartColorStr=\"" + c1.asHex() + "\",EndColorStr=\"" + c2.asHex() + "\")";
  } else {
    colorArray = BorderColor.createColorPath(c1,c2,Math.min(e.offsetWidth,50));
...
BorderColor.createGradientContainer = function() {
...
BorderColor.createColorPath = function(color1,color2,slices) {
  var colorPath = [];
  var colorPercent = 1.0;
  var delta=1.0/slices;
  do {
    colorPath[colorPath.length]=BorderColor.setColorHue(color1,colorPercent,color2);
    colorPercent-=delta;
  } while(colorPercent>0);
  return colorPath;
}

BorderColor.setColorHue = function(originColor,opacityPercent,maskRGB) {
...
Corner.prototype = {
   ...
   _borderColor : function(color,bgColor) {
      ...
      var cc1 = BorderColor.createFromHex(bgColor);
      var cc2 = BorderColor.createFromHex(color);
