 ...
 * @patches       1/4/08 by daniel boehringer: added registerDocumentForSelector and fixed show to hide all other open menus ($$('.'+this.options.className.split(' ')[0]).each(function(elem){elem.hide();});)
*/
...
Proto.Menu = Class.create({
	initialize: function() {
		...
		this.registerDocumentForSelector();
	},
	registerDocumentForSelector: function() {
	...
	show: function(e) {
		e.stop();
		$$('.'+this.options.className.split(' ')[0]).each(function(elem){elem.hide();});
		this.options.beforeShow(e);

        setTimeout(function(target){new Effect.Pulsate(target)}.bind(this,e.target),100);
